...
$(document).ready(function() {
    ...
    function init_template() {
        ...
        // for add value in url
        function setUrlParams(url, key, value) {
            url = url.split('?');
            let usp = new URLSearchParams(url[1]);
            usp.set(key, value);
            url[1] = usp.toString();
            return url.join('?');
        }
        // end for add value in url
        $(".sellMarginAll").on('click', function() {
            let url = $(this).attr("href");
            const id_product = new URLSearchParams(url).get('id_product');
            let getamount = $('#profitnya' + id_product).val();
            let url1 = setUrlParams(url, 'amount', getamount); // url = 'myapp.php?id=11&enabled=true'
            // console.log(url1);
            $("#modalSellMarginAll").find("#contentMarginSellAll").load(url1);

        });
        $(".sellMargin").on('click', function() {
            let url = $(this).attr("href");
            const id_product = new URLSearchParams(url).get('id_product');
            const margin_id = new URLSearchParams(url).get('id');
            let getamount = $('#profitnya' + id_product + margin_id).val();
            let url1 = setUrlParams(url, 'amount', getamount); // url = 'myapp.php?id=11&enabled=true'
            // console.log(url1);
            $("#modalSellMargin").find("#contentMarginSell").load(url1);
        ...
        $('.reloadermenu').on('click', function() {
            window.location.reload(true);
            caches.delete('workbox-runtime').then(function() {});
            localStorage.clear();
            sessionStorage.clear()
        ...
        if ($('.chart').length > 0) {
            ...
            var call_charts_to_page = function() {
                ...
                var walletChartProfit = $('#wallet-chart-profit');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        data.forEach((item) => {
                            ...
                            datadate[date.getDate()] = (date.getMonth() + 1) + '/' + date.getDate();
                            profit[date.getDate()] = { profit: parseFloat(item.profit).toFixed(4), profitdate: (date.getMonth() + 1) + '/' + date.getDate() };
                        })

                        let objectProfit = Object.values(profit)
                        let objectDatadate = Object.values(datadate)
                        var date = new Date();
                        var setEnd = date.getFullYear() + '/' + (date.getMonth() + 1) + '/' + (date.getDate() - 1);
                        date.setDate(date.getDate() - 14);
                        const start = date.getFullYear() + '/' + (date.getMonth() + 1) + '/' + date.getDate();
                        const end = new Date(setEnd);
                        ...
                        while (loop <= end) {
                            ...
                            let formatdate = (loop.getMonth() + 1) + '/' + loop.getDate();
                            datelist[formatdate] = formatdate;
                            // for find
                            function isProfit(item) {
                                return item.profitdate === formatdate;
                            }
                            if (objectProfit.find(isProfit)) {
                                chartProfit[loop] = objectProfit.find(isProfit).profit;
                            } else {
                                chartProfit[loop] = '0';
                            }
                        }
                        let objectDate = Object.values(datelist)
                        let objectChartProfit = Object.values(chartProfit)
                        var daterange = objectDate.concat(objectDatadate.filter((item) => objectDate.indexOf(item) < 0))

                        if (walletChartProfit.length) {
                            var walletDemoChart = new Chart(walletChartProfit, {
                                type: 'bar',
                                data: {
                                    labels: daterange,
                                    datasets: [{
